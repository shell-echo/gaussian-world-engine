import {RUNTIME_NAV_MISSION_DIAGNOSTICS_VERIFIER_EVIDENCE_VERIFICATION_RESULT_VERIFIER_EVIDENCE_VERIFICATION_RESULT_SCHEMA as RESULT_SCHEMA,RUNTIME_NAV_MISSION_DIAGNOSTICS_VERIFIER_EVIDENCE_VERIFICATION_RESULT_VERIFIER_EVIDENCE_VERIFICATION_RESULT_SCHEMA_VERSION as RESULT_VERSION} from "./NavMissionDiagnosticsVerifierEvidenceVerificationResultVerifierEvidenceVerificationResultContract.js";
import {RUNTIME_NAV_MISSION_DIAGNOSTICS_ARTIFACT_VERIFIER_EVIDENCE_VERIFICATION_RESULT_VERIFIER_EVIDENCE_SCHEMA as SCHEMA,RUNTIME_NAV_MISSION_DIAGNOSTICS_ARTIFACT_VERIFIER_EVIDENCE_VERIFICATION_RESULT_VERIFIER_EVIDENCE_SCHEMA_VERSION as VERSION} from "./NavMissionDiagnosticsArtifactVerifierEvidenceVerificationResultVerifierEvidenceContract.js";
...
import type {RuntimeNavMissionDiagnosticsArtifactVerifierEvidenceVerificationResultVerifierEvidenceVerificationChecks as Checks,RuntimeNavMissionDiagnosticsArtifactVerifierEvidenceVerificationResultVerifierEvidenceVerificationIssue as Issue} from "./NavMissionDiagnosticsArtifactVerifierEvidenceVerificationResultVerifierEvidenceVerificationContract.js";
import {SOURCE_ANCHOR_FIELDS,SOURCE_CHECK_FIELDS,SOURCE_EVIDENCE_FIELDS,add,booleanField,boolRecord,bounds,checksumField,exact,expectedIssueMessage,filenameField,integerField,record,safe,stringField,trustField,type Limits,type RecordValue} from "./NavMissionDiagnosticsArtifactVerifierEvidenceVerificationResultVerifierEvidenceVerificationSupport.js";
...
const RECORDED=["schema","schemaVersion","artifactVerifierEvidenceVerificationValid","artifactVerifierEvidenceVerificationTrust"] as const;
const RESULT=["valid","trust","issueCount"] as const;
const ISSUE=["code","path","message"] as const;
const MAX_ISSUES=512;

export function verifyRuntimeNavMissionDiagnosticsArtifactVerifierEvidenceVerificationResultVerifierEvidenceDocument(value:RecordValue,limits:Limits,issues:Issue[],checks:Checks):Document|null{
  ...
  return checks.schema&&checks.input&&checks.recordedResult&&checks.result&&checks.verificationChecks&&checks.anchors&&checks.evidence&&checks.issues?value as unknown as Document:null;
}

function verifySchema(value:RecordValue,issues:Issue[]):boolean{const start=issues.length;if(value.schema!==SCHEMA)add(issues,"schema-mismatch","$.schema","Verifier-evidence schema mismatch.");if(value.schemaVersion!==VERSION)add(issues,"schema-version-mismatch","$.schemaVersion","Verifier-evidence schema version mismatch.");return issues.length===start}
function verifyTarget(value:unknown,issues:Issue[]):void{if(value===null)return;const target=record(value,"$.target",issues);if(!target)return;exact(target,TARGET,"$.target",issues);if(target.scope!==null)stringField(target.scope,"$.target.scope",64,issues);if(target.packageIndex!==null)integerField(target.packageIndex,"$.target.packageIndex",issues)}
...
function verifyInput(value:RecordValue|null,issues:Issue[]):boolean{
  ...
  if(sum){exact(sum,CHECKSUM,"$.input.exactChecksum",issues);if(sum.algorithm!=="SHA-256")add(issues,"field-value-invalid","$.input.exactChecksum.algorithm","Algorithm must be SHA-256.");if(sum.input!=="artifact-verifier-evidence-verification-result-json-utf8")add(issues,"field-value-invalid","$.input.exactChecksum.input","Checksum input mismatch.");checksumField(sum.hex,"$.input.exactChecksum.hex",issues)}
  boolRecord(envelope,ENVELOPE,"$.input.envelope",issues);
  if(envelope){const expectedSafe=name!==null&&safe(name);if(envelope.filenameSafe!==expectedSafe||envelope.mimeTypeMatches!==(mime==="application/json;charset=utf-8")||envelope.byteSizeMatches!==(declared!==null&&declared===exactBytes)||envelope.checksumMatches!==(declaredHash!==null&&sum?.hex===declaredHash))add(issues,"input-envelope-mismatch","$.input.envelope","Input envelope booleans are inconsistent.")}
...
function verifyRecorded(value:RecordValue|null,issues:Issue[]):boolean{
  ...
  if(value.schema!==null&&value.schema!==RESULT_SCHEMA)add(issues,"recorded-result-mismatch","$.recordedResult.schema","Recorded artifact-verifier evidence verification-result schema mismatch.");
  if(value.schemaVersion!==null&&value.schemaVersion!==RESULT_VERSION)add(issues,"recorded-result-mismatch","$.recordedResult.schemaVersion","Recorded artifact-verifier evidence verification-result schema version mismatch.");
  if(value.artifactVerifierEvidenceVerificationValid!==null)booleanField(value.artifactVerifierEvidenceVerificationValid,"$.recordedResult.artifactVerifierEvidenceVerificationValid",issues);
  if(value.artifactVerifierEvidenceVerificationTrust!==null)trustField(value.artifactVerifierEvidenceVerificationTrust,"$.recordedResult.artifactVerifierEvidenceVerificationTrust",issues);
  return issues.length===start;
}
...
function verifyResult(value:RecordValue|null,issues:Issue[]):boolean{
  ...
  const valid=booleanField(value.valid,"$.result.valid",issues),trust=trustField(value.trust,"$.result.trust",issues),count=integerField(value.issueCount,"$.result.issueCount",issues);
...
function verifyIssues(entries:unknown[]|null,issues:Issue[]):boolean{
  ...
  for(let index=0;index<entries.length;index++){
    ...
    if(code&&expected===null)add(issues,"issue-evidence-mismatch",`$.issues[${index}].code`,"Issue code is not part of the fixed 0.99 verifier contract.");
    else if(expected!==null&&message!==expected)add(issues,"issue-evidence-mismatch",`$.issues[${index}].message`,"Issue message does not match the deterministic verifier-evidence mapping.");
  }
  return issues.length===start;
}

function verifyRelationships(result:RecordValue|null,sourceChecks:RecordValue|null,anchors:RecordValue|null,evidence:RecordValue|null,entries:unknown[]|null,verification:Checks,issues:Issue[]):void{
  ...
  if(valid===true)for(const field of SOURCE_CHECK_FIELDS)if(sourceChecks[field]!==true){verification.verificationChecks=false;add(issues,"verification-check-mismatch",`$.checks.${field}`,"Recorded valid result requires every artifact-verifier verification-result check.")}
