...
async function writeColmapRunner(args: string[]): Promise<void> {
  const sessionPath = path.resolve(requiredArg(args, 0, "session.json"));
  const session = await readSession(sessionPath);
  const root = path.dirname(sessionPath);
  const framePlan = createFramePlan(session, root, sessionPath);
  ...
  await mkdir(path.join(root, "poses", "colmap", "sparse"), { recursive: true });
  await mkdir(path.join(root, "poses", "colmap", "model-text"), { recursive: true });
  await writeJson(path.join(root, "frames", "frame-plan.json"), framePlan);
  await writeJson(jobPath, job);
  await writeJson(planPath, plan);
  await writeFile(scriptPath, createColmapScript(plan), "utf8");
  await writeJson(reportPath, createColmapRunnerReport(plan));
...
function createColmapScript(plan: ColmapRunnerPlan): string {
  const lines = [
    "#!/usr/bin/env bash",
    "set -euo pipefail",
    "",
    "# Generated by swe-builder. Run from the capture project root after frame extraction.",
    `mkdir -p ${shellQuote(plan.workDir)}`,
...
function shellCommand(tokens: readonly string[]): string {
