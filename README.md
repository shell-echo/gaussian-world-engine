# Splat World Engine — Verification Result Verifier Evidence Verification-Result Artifact Verification

一个 **Gaussian-first、Mesh-assisted** 的浏览器游戏 Runtime / Builder 原型。Runtime/Builder 0.95 为 0.94 verification-result 三件套增加独立 verifier，直接审计 result JSON、canonical bytes、recorded 0.93 verifier result、trusted anchors、text result、artifact envelope 与 JSON SHA-256，不调用 0.94 creator 重新生成 expected output。

## Runtime/Builder 0.95

核心 API：

```ts
verifyRuntimeNavMissionDiagnosticsVerifierEvidenceVerificationResultText(text, options)
verifyRuntimeNavMissionDiagnosticsVerifierEvidenceVerificationResultTextAnchored(text, options)
verifyRuntimeNavMissionDiagnosticsVerifierEvidenceVerificationResultArtifact(
  source,
...
Verifier 覆盖 fixed 0.94 schema/version、unknown fields、recursive canonical JSON、0.92 verifier-evidence input exact UTF-8 bytes/SHA-256、recorded 0.92 schema/version 与 historical 0.91 verification-result artifact verifier result、recorded 0.93 `valid/trust/checks/anchors/issues`、stable issue evidence、text result、fixed artifact order、safe filenames、MIME、UTF-8 byte sizes、per-artifact SHA-256、totalBytes 与 strict `.sha256` syntax。
...
0.94 document 中的 `result.valid/result.trust` 是 0.93 verifier-evidence verifier 的历史结论；0.95 返回值描述 0.94 verification-result artifact set 本身是否可信。因此，准确记录 `invalid / untrusted` 历史验证结果的 0.94 三件套仍可成为 anchored 或 self-consistent 的失败审计证据。
...
0.94 text 与 `.sha256` artifacts 只证明 envelope consistency，不作为 external authority。
...
安全边界：verification 不调用 0.94 creator、不执行输入、不使用 `innerHTML`、不从 filename 创建路径、不自动下载、创建 Blob URL 或写 Clipboard。0.95 继续使用短内部 module filename family `NavMissionDiagnosticsVerifierEvidenceVerificationResultVerification*`，避免递归链重新引入常见 255-byte 单文件名限制风险，同时保留完整公开 API 与 schema 语义。

```text
package version: 0.95.0
runtime label: runtime 0.95
```
...
- [ ] Verification-result artifact-verifier evidence artifacts

下一版建议把 0.95 verifier result 固化为 deterministic text、canonical JSON 与 JSON SHA-256 verifier-evidence artifacts；creator 仍只记录已完成的 0.95 verifier 结论，不重新执行 verifier，也不调用 0.94 creator 作为 authority。
